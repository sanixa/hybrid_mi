...
class Loss(torch.nn.Module):
    ...
    def forward(self, z, x_gt_ch3):
        ...
        self.gen_img_ch3 = self.netG(z).view(batchsize, 3, self.args.resolution, self.args.resolution)
...
def main():
    ...
    netG = model.Generator_celeba(1).cuda()
    ...
    init_val = init_val.reshape(args.data_num, Z_DIM, 1, 1)
    ...
    trainset = CelebA(root=os.path.join('/work/u5366584/exp/datasets/celeba'), split='train', ## ../intern/GS-WGAN-custom/exp/datasets/celeba
        transform=transform_train, download=False) 
    testset = CelebA(root=os.path.join('/work/u5366584/exp/datasets/celeba'), split='test', ## ../intern/GS-WGAN-custom/exp/datasets/celeba
        transform=transform_train, download=False)


    indices = np.loadtxt('index_80k.txt', dtype=np.int_)
