...
class Loss(torch.nn.Module):
    ...
    def forward(self, z, x_gt):
        ...
        gen_img_ch3 = gen_img_ch3.float()
        x_gt_ch3 = x_gt_ch3.float()
        self.loss_lpips = self.loss_lpips_fn(gen_img_ch3, x_gt_ch3)
        self.loss_l2 = self.loss_l2_fn(gen_img_ch3, x_gt_ch3)

        loss_l2_confident_vector = torch.mean((self.netC(gen_img_ch3) - self.netC(x_gt_ch3)) ** 2, dim=[1])
            ...
            if os.path.exists(save_dir_batch) and False:
                ...
                for step in range(max_func):
                    ...
                    if step % (max_func//10) == 0:
                        ### store init
                        x_hat_curr = loss_model.gen_img.data.cpu().numpy()
