...
def plot_roc(pos_results, neg_results):
    ...
    prec = metrics.precision_score(labels, pred_labels)
    recall = metrics.recall_score(labels, pred_labels)
    f1 = metrics.f1_score(labels, pred_labels)
    b_acc = metrics.balanced_accuracy_score(labels, pred_labels)
    ...
    return fpr, tpr, threshold, ap, auc, acc, prec, recall, f1, b_acc, tpr_thres, fpr_thres
...
def main():
    ...
    ### plot roc curve
    fpr, tpr, threshold, ap, auc, acc, prec, recall, f1, b_acc, tpr_thres, fpr_thres = plot_roc(-pos_loss, -neg_loss)
    plt.plot(fpr, tpr, label='%s attack, auc=%.3f, ap=%.3f' % (attack_type, auc, ap))

    logger.info('best_auc, best_acc, best_prec, best_recall, best_f1, best_b_acc, best_tpr_thres, best_fpr_thres: {}, {}, {}, {}, {}, {}, {}, {}'.format(auc, acc, prec, recall, f1, b_acc, tpr_thres, fpr_thres))
    ...
    if reference_load_dir is not None:
        ...
        fpr, tpr, threshold, ap, auc, acc, prec, recall, f1, b_acc, tpr_thres, fpr_thres = plot_roc(-pos_calibrate, -neg_calibrate)
        plt.plot(fpr, tpr, label='calibrated %s attack, auc=%.3f, ap=%.3f' % (attack_type, auc, ap))
        
        logger.info('Calibrated: best_auc, best_acc, best_prec, best_recall, best_f1, best_b_acc, best_tpr_thres, best_fpr_thres: {}, {}, {}, {}, {}, {}, {}, {}'.format(auc, acc, prec, recall, f1, b_acc, tpr_thres, fpr_thres))
